...
Create a symbolic link that points `/usr/local/mysql` to `/usr/local/mysql-VERSION`.
	...
	sudo ln -s mysql-VERSION /usr/local/mysql
...
If you are copying the data from a former MySQL installation, you can skip this step. Initialize your database installation.
	...
	sudo /usr/local/mysql/scripts/mysql_install_db --basedir=/usr/local/mysql --datadir=/usr/local/var/mysql
...
To start the MySQL server.
	...
	sudo -b /usr/local/mysql/bin/mysqld_safe --user=root --datadir=/usr/local/var/mysql
